...
s_bit_field(6, 24, fuzzable=False) # length(bytes number) of payload in frame
...
# payload
s_bit_field(2, 16, fuzzable=False) # settings enable_push
s_bit_field(0, 32, fuzzable=False) # disable

########################################################################################################################
# Settings packet for ACK in HTTP/2 protocol
########################################################################################################################
s_initialize("HTTP/2 Settings ACK")
s_bit_field(0, 24, fuzzable=False) # length(bytes number) of payload in frame
s_bit_field(4, 8, fuzzable=False)  # type of frame
...
def set_header_frame(headers):
    ...
    s_size("headers payload", length=3, format="binary") # length(bytes number) of payload in frame
    s_bit_field(1, 8, fuzzable=False)  # type of frame
    s_bit_field(4, 8, fuzzable=False)  # flags of frame, End Headers
    s_bit_field(1, 32, fuzzable=False, name="header stream id") # stream ID
    # payload (Header block fragment)
    if s_block_start("headers payload"):
        e = Encoder()
        s_string(e.encode(headers), encoding="binary", fuzzable=False)
        """
...
s_bit_field(1, 32, fuzzable=False, name="data stream id") # stream ID
# payload (Header block fragment)
